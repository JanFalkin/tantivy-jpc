...
use cbindgen::{
    Config, ExportConfig, ItemType, Language, MangleConfig, ParseConfig, ParseExpandConfig,
    Profile, RenameRule,
};
use std::env;
...
fn main() {
    ...
    let output_secondary_header = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
        .join(format!(
            "go-client/tantivy/packaged/include/{package_name}.h"
        ))
        .display()
        .to_string();
    ...
    if std::fs::copy(&output_file, &output_secondary_header).is_err() {
        panic!(
            "header file copy {} to {} FAILED",
            &output_file, &output_secondary_header
        );
    }
}
