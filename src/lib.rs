...
// Each TantivySession will lazily fill its Options over its lifetime and each will be a unique
...
impl<'a> TantivySession<'a> {
    ...
    pub fn do_method(
        ...
        let _ = &self.builder;
        (
            self.return_buffer.as_ptr() as *const u8,
            self.return_buffer.len(),
            false,
        )
    }
}
...
pub unsafe extern "C" fn tantivy_jpc(
    ...
    // REVIEW: Not sure if this is my misunderstanding, but with this implementation, I dont see how
    // you can have multiple tantivy sessions running concurrently. It seems like the mutex is
    // grabbed here, and would still be held until `tm` goes out of scope, which is after
    // `entity.do_method` is called.  Just something I'm a touch confused about
    let mut tm: std::sync::MutexGuard<HashMap<String, TantivySession>> = match TANTIVY_MAP.lock() {
        Ok(t) => t,
        Err(e) => {
            info!("TANTIVY_MAP lock failed {e}");
            return -1;
    ...
    let (return_val, ret_sz, is_error) =
